      ...
      <div class="bg-white rounded-2xl max-w-3xl w-full max-h-[90vh] my-auto flex flex-col shadow-2xl border border-slate-100 overflow-hidden">
        ...
        <div class="print-only border-b-2 border-slate-800 pb-4 mb-4">
          <div class="flex justify-between items-start">
            <div>
              <h1 class="text-xl font-black text-slate-900 tracking-wider">SGI PRO SOLUCIONES S.A.S.</h1>
              <p class="text-xs text-slate-600">NIT: 901.458.789-1 | Régimen Común - Responsable de IVA</p>
              <p class="text-xs text-slate-600">Dirección: Cra 7 # 71-21 Torre B, Piso 8 - Bogotá D.C., Colombia</p>
              <p class="text-xs text-slate-600">PBX: +57 (1) 745-0000 | Email: facturacion&#64;sgipro.com</p>
            </div>
            <div class="text-right">
              <div class="inline-block border-2 border-slate-900 px-4 py-2 rounded-lg text-center">
                <span class="block text-[10px] font-bold uppercase tracking-widest text-slate-600">Factura de Venta</span>
                <span class="block text-base font-black text-slate-900 font-mono">{{ sale.saleNumber }}</span>
              </div>
              <p class="text-[10px] text-slate-500 mt-1">Res. DIAN 18764000129345</p>
            </div>
          </div>
        </div>
          ...
          <div class="flex items-center gap-3">
            ...
            <div>
              <div class="flex items-center gap-2">
                <h2 class="text-base font-bold text-slate-900">{{ sale.saleNumber }}</h2>
                <span
                ...
                >
                  {{ sale.status === 'Completed' ? 'Completada' : 'Anulada / Cancelada' }}
                </span>
              </div>
              <p class="text-xs text-slate-500">Facturada el {{ sale.saleDate | date:'mediumDate' }} por {{ sale.userName }}</p>
            </div>
          </div>
          <div class="flex items-center gap-2">
            <button
              type="button"
              (click)="printInvoice()"
              class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-slate-200 bg-white hover:bg-slate-50 text-slate-700 text-xs font-semibold shadow-sm transition-all cursor-pointer"
          ...
          <div>
            @if (sale.status === 'Completed') {
              <app-button variant="danger" size="sm" (clicked)="onCancelSale()">
                ✕ Anular Venta y Restituir Stock
              </app-button>
            }
          </div>
          <div class="flex items-center gap-2">
            <app-button variant="secondary" size="sm" (clicked)="printInvoice()">
              🖨️ Imprimir
            </app-button>
